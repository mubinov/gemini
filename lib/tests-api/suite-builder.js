...
module.exports = function(suite) {
    ...
    this.before = function(hook) {
        ...
        hook.call(suite.context, new ActionsBuilder(suite.beforeActions), find);
    ...
    this.after = function(hook) {
        ...
        var actions = [];
        hook.call(suite.context, new ActionsBuilder(actions), find);
        suite.afterActions = actions.concat(suite.afterActions);

        return this;
    };

    this.setRootUrl = function setRootUrl(rootUrl) {
        if (typeof rootUrl !== 'string') {
            throw new TypeError('Root URL must be string');
        }
        suite.rootUrl = rootUrl.replace(/\/$/, '');
        return this;
    };
    ...
    this.capture = function capture(name, opts, cb) {
        ...
        cb.call(suite.context, new ActionsBuilder(state.actions), find);
