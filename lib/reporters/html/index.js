...
function logPathToHtmlReport() {
    const reportPath = `file://${path.resolve((lib.getReportDir()) + '/index.html')}`;

    logger.log(`Your HTML report is here: ${chalk.yellow(reportPath)}`);
}

function setReporterPath(reporterPath) {
    lib.setReportDir(reporterPath);
}
...
module.exports = function htmlReporter(runner, reportPath) {
    const generateReportPromise = Promise.all([setReporterPath(reportPath), prepareViewData(runner), prepareImages(runner)])
        .spread(view.createHtml)
        .then((html) => view.save(html))
